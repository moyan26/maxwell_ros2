...
public:
  VisionNode() : rclcpp::Node("vision_node")
  {
    ...
    if (!VISION->start())
    {
      RCLCPP_FATAL(this->get_logger(), "VISION->start() failed. Check net_cfg_file/net_weights_file and CUDA/Darknet setup.");
...
private:
  ...
  void on_timer()
  {
    ...
    out.header.stamp = (last_header_stamp_.sec == 0 && last_header_stamp_.nanosec == 0) ? this->now() : last_header_stamp_;
